		...
		<Root className={classes.root} reset={resetPassword?.toString()}>
			<Container component="main" maxWidth="xs">
				<Paper className={classes.paper} elevation={6} style={{ marginTop: '260px' }}>
					...
					<Typography variant="h5">{resetPassword ? 'Redefinir senha' : 'Recuperar conta'}</Typography>
					<form className={classes.form} onSubmit={handleSubmit}>
						<Grid container spacing={1}>
							{resetPassword ? ( //
								<Input name="newPassword" label="Nova Senha" onChange={handleChange} type={showPassword ? 'text' : 'password'} handleShowPassword={handleShowPassword} />
							) : (
								<Input name="email" label="Endereço de email" onChange={handleChange} type="email" />
							)}
						</Grid>
						<Button type="submit" fullWidth variant="contained" color="primary" className={classes.submit}>
							{resetPassword ? 'CONFIRMAR' : 'ENVIAR LINK DE REDEFINIÇÃO DE SENHA'}
						</Button>
					</form>
				</Paper>
			</Container>
