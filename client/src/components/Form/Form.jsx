...
const Form = ({ currentId, setCurrentId, user, snackBar }) => {
	...
	const [fileName, setFileName] = useState('Nenhuma postagem selecionada')
	...
	useEffect(() => {
		if (post) {
			...
			setTags(post.tags)
			setFileName('Imagem Anterior')
			setOldLabel('Imagem Anterior')
	...
	const setMediaEmpty = () => {
		setMedia(null)
		setFileName('Nenhuma postagem selecionada')
	}
			...
			<Root className={classes.root}>
				<Paper className={classes.paper} elevation={6}>
					<Typography variant="h6" align="center">
						Faça login crie e compartilhe bons momentos da vida. ✍️📸
					</Typography>
				</Paper>
			</Root>
		)
			...
			<Paper className={classes.paper} elevation={6}>
				<form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
					<Typography style={{ color: 'white', textAlign: 'center' }} variant="h6">
						{currentId ? `Editing ${post.title}` : 'Crie um momento'}
					</Typography>
					...
					<TextField sx={{ input: { color: 'white' } }} name="title" variant="outlined" label="Título" fullWidth value={postData.title} onChange={(e) => setPostData({ ...postData, title: e.target.value })} />
					<TextField InputProps={{ style: { color: 'white' } }} name="message" variant="outlined" label="Mensagem" fullWidth multiline rows={4} value={postData.message} onChange={(e) => setPostData({ ...postData, message: e.target.value })} />
					<ChipInput fullWidth InputProps={{ style: { color: 'white' } }} value={postData.tags} newChipKeyCodes={[188, 13]} onAdd={handleAdd} onDelete={handleDelete} label="Etiquetas" variant="outlined" className={classes.chip} />
					<Button className={classes.buttonSubmit} disabled={validate} variant="contained" color="primary" type="submit" fullWidth>
						{currentId ? 'Atualizar' : 'Criar'}
					</Button>
					<Button className={classes.buttonSubmit} variant="contained" color="secondary" onClick={clear} fullWidth>
						{currentId ? 'CANCELAR' : 'LIMPAR'}
					</Button>
				</form>
			</Paper>
		</Root>
